...
from lambda_function.response_builder import (build_discovery_response,
...
class ResponseBuilderTest(unittest.TestCase):
    ...
    @mock.patch('lambda_function.response_builder.uuid')
    def test_build_discovery_response(self, mock_uuid):
        mock_uuid.uuid4.return_value = '16120434-3e91-4b65-a2ee-f30cbb5fa1f6'

        with open('tests/json/cloud-api-discovery-response-01.json') as f:
            cloud_api_discovery_endpoint = json.load(f)
        with open('tests/json/alexa-discovery-response-01.json') as f:
    ...
    def test_build_power_controller_response(self, mock_uuid):
        ...
        with open('tests/json/alexa-power-controller-response-ON.json') as f:
            alexa_power_controller_expected_on = json.load(f)

        with open('tests/json/alexa-power-controller-response-OFF.json') as f:
    ...
    def test_build_input_controller_response(self, mock_uuid):
        ...
        with open('tests/json/alexa-input-controller-response-HDMI1.json') as f:
            alexa_input_controller_expected_hdmi1 = json.load(f)

        with open('tests/json/alexa-input-controller-response-XBOX.json') as f:
