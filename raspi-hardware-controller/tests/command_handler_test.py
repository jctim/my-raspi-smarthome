...
# TODO add mqtt reply expectations
...
class CommandHandlerTest(unittest.TestCase):
    ...
    def test_handle_control_command_power(self, mock_os):
        handle_control_command({'power': 'on', 'uuid': _uuid()}, None)
        mock_os.system.assert_called_with('echo "tx 10:04" | cec-client RPI -s -d 1')

        handle_control_command({'power': 'off', 'uuid': _uuid()}, None)
    ...
    def test_handle_control_command_source_hdmi_numbers(self, mock_os):
        handle_control_command({'source': 'HDMI 1', 'uuid': _uuid()}, None)
    ...
    def test_handle_control_command_source_custom_names(self, mock_os):
        ...
        [handle_control_command({'source': source, 'uuid': _uuid()}, None) for source in hdmi1_names]
        ...
        hdmi2_names = ['XBOX']
        [handle_control_command({'source': source, 'uuid': _uuid()}, None) for source in hdmi2_names]
        ...
        [handle_control_command({'source': source, 'uuid': _uuid()}, None) for source in hdmi3_names]
        ...
        [handle_control_command({'source': source, 'uuid': _uuid()}, None) for source in hdmi4_names]
